...
window.onmouseover = function(e){
    ...
    let tagfinder =e.target.tagName 
    let text='', textresult='';
 
    console.log(textresult)
    switch (tagfinder) {
       case 'SPAN':
           text= e.target.parentElement.parentElement.parentElement.previousElementSibling.innerText
           console.log(text)
           textresult  =  text.replace(spacebar, ' ')
           console.log(textresult)
           document.querySelector('.data_txt .txt2').innerHTML = textresult;
           break;
       case 'EM':
           text= e.target.parentElement.parentElement.parentElement.previousElementSibling.innerText
           console.log(text)
           ...
           break;
       case 'LI':
         text = e.target.parentElement.previousElementSibling.innerText
         console.log(text)
         textresult  =  text.replace(spacebar, ' ')
         console.log(textresult)
         document.querySelector('.data_txt .txt2').innerHTML = textresult;
...
/* footer */
//HTML 문서가 완전히 로드된 후에 함수 내의 코드를 실행
    // 마우스 클릭 시 계열사 메뉴 보이기
